...
#[test]
fn test_common_patterns_sequential() {
    assert!(check_common_patterns("abc123xyz"));
}

#[test]
fn test_common_patterns_repeated() {
    assert!(check_common_patterns("aaabbb"));
}

#[test]
fn test_common_patterns_keyboard() {
    assert!(check_common_patterns("qwertyPass1"));
}

#[test]
fn test_common_patterns_false() {
    assert!(!check_common_patterns("MySecureP@ss9"));
}
...
fn test_entropy_positive() {
    assert!(calculate_entropy("Password123!") > 0.0);
}
